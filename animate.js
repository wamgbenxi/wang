function myAnimate (obj,attrObj,dur,fn,callback,obj2,obj3) {
      var tween;
	  var callfun;
   ...
   obj.time=setInterval(function  () {
     ...
     if(times<dur){
	  ...
	  if(obj2){
	   if((obj.offsetTop>obj2.offsetTop&&obj.offsetTop<obj2.offsetTop+obj2.offsetHeight)&&
		 (obj.offsetLeft>obj2.offsetLeft&&obj.offsetLeft<obj2.offsetLeft+obj2.offsetWidth)  
	     ){
		   
	       obj2.close=false;
		   var dddd=document.getElementsByTagName("div");
		   for (var i=0; i<dddd.length; i++) {
			   if(dddd[i].time){
			   clearInterval(dddd[i].time)
               dddd[i].time=null;
			   }
		   }
		    Queue.wait(again,"fx",2000);
